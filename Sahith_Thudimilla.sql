...
SELECT (EXTRACT(YEAR FROM TO_DATE("PURCHASE_DATE",'YYYY-MM-DD'))) AS YEAR ,SUM(price*quantity) as totalsale FROM ecommerce
...
SELECT country,COUNT(customerno) AS No_of_Customers FROM ecommerce
GROUP BY country;
